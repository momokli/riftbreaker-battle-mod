...
| [07-relay](07-relay/) | Relay-Brücke: tailt `exor_logs.txt` ([RBBATTLE]-Zeilen), liefert Events per `POST /event` beim 06-Server ein (In-Memory-Queue, Backoff-Retry), Poll-Loop dispt `exec_command` (v0: loggt „dispatch pending“, echtes `dispatch_exec` via Pipe/rbbridge folgt) | untested — Unit-Tests grün, In-Game-Test offen | `cd bausteine/07-relay && python3 -m unittest test_dispatch test_referee -v` (Pipe-Dispatch + Referee-Rückkanal, ohne Spiel) |
| [08-web-ui](06-tournament-server/web/) | Control-Dashboard im Terminal-Stil (vom 06-Server aus `web/` statisch serviert): PLAYERS + EVENT FEED (live via SSE `/stream`) + CONTROLS (`rb_wave`/`rb_points`/`rb_buy_wave` … als `exec_command`), localStorage, vanilla JS | entfernt mit Baustein 06 (Green-Field-Refaktorierung) | — |
...
- [x] 07: `python3 -m unittest test_dispatch test_referee -v` grün (Pipe-Dispatch + Referee-Rückkanal, ohne Spiel). Der frühere 07+08-Durchstich (`test_e2e_prototype.sh`) hing am mit Baustein 06 entfernten Node-Tournament-Server und wurde entfernt.
