...
/* #387 Host-Test: LuaGraphNode::SetSuspended(bool) ist Windows-x64-Code
 * (mov [rcx+0xF1], dl) und unter der SysV-ABI des Host-Compilers nicht
 * ausfuehrbar. Der Shim zeichnet den Aufruf auf und schreibt das
 * suspended-Flag (+0xF1) direkt in den (vom Test bereitgestellten) Puffer. */
static void *g_ht_suspend_inst = NULL;
static int   g_ht_suspend_on = -1;
static int   g_ht_suspend_calls = 0;
static void ht_call_set_suspended(uint64_t fn, void *inst, int on)
{
    (void)fn;
    g_ht_suspend_inst = inst;
    g_ht_suspend_on = on;
    g_ht_suspend_calls++;
    if (inst)
        ((unsigned char *)inst)[0xF1] = (unsigned char)(on ? 1 : 0);
}
#define RBBRIDGE_SET_SUSPENDED(fn, inst, on) \
    ht_call_set_suspended((fn), (inst), (on))
...
/* #387: SetSuspended-Aufruf. Host-Test: Aufzeichnungs-Shim (s. oben, SysV
 * statt x64-ABI); Produktion: direkter Call ueber den aufgeloesten Zeiger. */
#ifndef RBBRIDGE_HOSTTEST
#define RBBRIDGE_SET_SUSPENDED(fn, inst, on) \
    (((void (*)(void *, int))(uintptr_t)(fn))((void *)(inst), (on)))
#endif
                                     ...
                                     uint64_t *out_setfn)
{
    ...
    *out_count = (int)n;
    ...
    /* Boot-/World-Guard: Instanz + Flagbyte muessen lesbar sein. */
    {
        ...
        if (!dom_boot_guard_ok((const unsigned char *)(uintptr_t)inst))
            return "world_not_ready";
        (void)flag;
    }

    RBBRIDGE_SET_SUSPENDED(*out_setfn, (void *)(uintptr_t)inst, suspend ? 1 : 0);
    *out_inst = inst;
    return NULL;
}
