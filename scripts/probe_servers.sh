...
#   "UDP-connect ok". Ohne Container (kein docker/kein Name) → null,
#   nicht false — ein container-loser Endpoint ist ungeprüft, nicht tot.
...
# Rückgabe: true | false | null (null = docker/Container-Name nicht
# verfügbar bzw. unbekannt — ungeprüft, NICHT "tot"; containerUp behandelt
# denselben Fall ebenfalls als null, s.u.).
udp_bound(){
  local ctr="$1" port="$2" out hex
  if ! command -v docker >/dev/null 2>&1; then echo null; return; fi
  if [ -z "$ctr" ]; then echo null; return; fi
...
site_check(){
  ...
  # curl liefert bei Fehler "000" — als Zahl ``000`` waere das ungueltiges
  # JSON. Auf 0 normalisieren, damit status.json immer valides JSON bleibt.
