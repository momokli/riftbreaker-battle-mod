...
class TestParseSendOrder(unittest.TestCase):
    ...
    def test_rbsendchat(self):
        line = "[RBBATTLE:20260917-market-wave] event=chat_send status=ok text=-send wave9"
        self.assertEqual(parse_send_order(line), "wave9")

    def test_leading_whitespace_name(self):
        line = "[RBBATTLE] button_chat sent: -send   wave2"
        self.assertEqual(parse_send_order(line), "wave2")
...
class TestSendTailer(unittest.TestCase):
    ...
    def test_feed_http_error_returns_name_still(self):
        ...
        tailer = self._tailer(lambda name: (503, '{"ok":false}'))
        self.assertEqual(tailer.feed("[RBBATTLE] button_chat sent: -send wave1"), "wave1")
