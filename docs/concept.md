...
Enthält **nur die Spiellogik** (Wellen spawnen, Punkte verwalten, Defense bauen, HUD) — **kein eigener I/O-Kanal** (Server-only, s. u.). Registriert Console-Commands (z. B. `rb_wave`) als Eingabe-API für die Server-DLL. Läuft komplett auf der offiziellen Mod-API ohne externe Zugriffe und bleibt damit **update-fest** und **Steam-Workshop-tauglich** — Game-Updates können dem Mod nur dann schaden, wenn sie die API selbst ändern (unwahrscheinlich, da offiziell).

### Server / Sidecar (`server/`, Windows-first)

Die **einzige I/O-Schicht** zwischen Spiel und Netz (Server-only, Entscheidung 08.09.2026). Ein externer **Injector** hängt die **Server-DLL** zur Laufzeit in den Spielprozess (runtime-only Injection); die DLL öffnet eine **Named Pipe** (z. B. `\\.\pipe\rbbattle`) zum lokalen Relay-Client. Zwei Aufgaben:
...
### Warum Server als I/O-Schicht statt Log-Bridge oder Konsole?
...
| **Server / Sidecar** (Memory-I/O) | ✅ | ✅ | Ein Kanal für beide Richtungen; kein User-Zutun; State-Reads direkt am Prozess |

→ Der Server ist die **einzige Option mit echtem Inbound ohne Spieler-Interaktion** und liest den State außerdem direkter und robuster, als jede Log- oder Konsolen-Brücke es könnte. Dafür ist er update-empfindlicher — abgefangen durch die update-feste Mod-Logik (siehe Risiken).
...
- **Spiellogik komplett im Lua-Mod:** Der Server bleibt ein „dummer“ Kanal. Logik-Änderungen sind damit reine Mod-Änderungen ohne Neukompilieren des Servers.

## Tournament-Architektur (Server-only)
...
**Leitprinzip (Entscheidung 08.09.2026): Server-only.** Die Lua-Mod hat **keinen eigenen I/O-Kanal** — Ingress UND Egress laufen ausschließlich über die **Server-DLL**:

```
┌────────────────────────────────┐
│  Rift Breaker + Lua-Mod        │
│  + Server-DLL (injiziert)     │
└────────────────┴───────────────┘
...
- **Server-DLL (`server/`, Windows-first):** das **einzige I/O-Gateway** zwischen Spiel und Außenwelt. Ein **Injector** (externes Tool) hängt die DLL zur Laufzeit in den Spielprozess; die DLL öffnet eine **Named Pipe** (z. B. `\\.\pipe\rbbattle`) zum lokalen Relay-Client.
...
**Ingress (Server → Spiel):** `Tournament-Server → Relay-Client → Named Pipe → Server-DLL → Mod-Command / Spielfunktion`

**Egress (Spiel → Server):** `Server-DLL (Game-State-Read / Event-Hook) → Named Pipe → Relay-Client → Tournament-Server`
...
- Die **Server-DLL ist bewusst ein externes Tool** („für uns“-Projekt): separate, private Distribution, kein Teil des Workshop-Uploads.
...
2. **Server-Harness** — Injector + DLL-Grundgerüst + Named Pipe *(läuft)*
...
- **Game-Updates brechen Server-Signaturen:** Mod-Logik bleibt davon unberührt; Offsets/Signaturen des Servers müssen nach jedem Patch geprüft werden. Der Server sollte AOB-Signaturen statt fester Adressen nutzen.
- **macOS-Server deutlich aufwendiger:** SIP + Hardened Runtime — bewusst vertagt (Windows-first).
- **Server nicht Workshop-fähig:** Die Server-DLL (In-Process-Injection) verstößt gegen die Plattform-Regeln — bewusst nur private Distribution („für uns“-Projekt). Die Mod selbst bleibt ein normaler Workshop-Mod; runtime-only Injection modifiziert keine Game-Dateien.
